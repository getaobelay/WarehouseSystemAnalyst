...
{
    public class CollectAllocationCommandHandler<TEntity, TDto, TCommand> : IRequestHandler<TCommand, ICommandResponse<TDto>>
        where TEntity : class, IBaseWarehouse, new()
        where TDto : class, IBaseWarehouseDto, new()
        where TCommand : CollectionCommand<TEntity, TDto>, new()
    {
